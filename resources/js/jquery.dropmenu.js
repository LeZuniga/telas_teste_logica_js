		...
		$curobj.hover(
				...
				$targetul.data('timers').showtimer=setTimeout(function(){
					...
					if ($targetul.queue().length<=1){ //if 1 or less queued animations
						...
						if (smoothmenu.shadow.enable && !smoothmenu.css3support){
							var shadowleft=header.istopheader? $targetul.offset().left+dropmenu.shadow.offsetx : menuleft
							var shadowtop=header.istopheader?$targetul.offset().top+smoothmenu.shadow.offsety : header._shadowoffset.y
							if (!header.istopheader && dropmenu.detectwebkit){ //in WebKit browsers, restore shadow's opacity to full
								header.$shadow.css({opacity:1})
							}
							header.$shadow.css({overflow:'', width:header._dimensions.subulw+'px', left:shadowleft+'px', top:shadowtop+'px'}).animate({height:header._dimensions.subulh+'px'}, dropmenu.transition.overtime)
						}
					}
			...
			function(e){
				...
				$targetul.data('timers').hidetimer=setTimeout(function(){
					...
					if (smoothmenu.shadow.enable && !smoothmenu.css3support){
						if (dropmenu.detectwebkit){ //in WebKit browsers, set first child shadow's opacity to 0, as "overflow:hidden" doesn't work in them
							header.$shadow.children('div:eq(0)').css({opacity:0})
						}
						header.$shadow.css({overflow:'hidden'}).animate({height:0}, dropmenu.transition.outtime)
					}
